...
void _check_zero_hold(const std::pair<const heyoka::taylor_adaptive<double> &, const heyoka::taylor_adaptive<double> &> &tas) {
    ...
    if (!(ta_var.get_state().size() == 77)) {
         throw std::domain_error(
            "The zero-hold variational Taylor adaptive integrator in the high fidelity sf leg, does not have a dimension of 77.");
    }
    // 2) the ta_var must have the same parameters as ts
    if (ta.get_pars().size() != ta_var.get_pars().size()) {
         throw std::domain_error(
            "The zero-hold Taylor adaptive integrator and its variational counterpart seem to have different number of parameters?");
    }
    // 3) the ta_var is a variational integrator
    if (!ta_var.is_variational()) {
        throw std::domain_error(
            "The zero-hold variational Taylor adaptive integrator in the high fidelity sf must be variational");
    }
    return;
}
// Utilty
